...
import { FormerGoogleApiService } from '../services/former-google-api.service';
...
export class AuthEffects {
  constructor(
    ...
    private googleApiService: FormerGoogleApiService,
  ...
  */
  ...
  loginAction$ = this.actions$.pipe(
    ...
    switchMap((payload) =>
      this.authService.emailLogin(payload.email, payload.password).pipe(
        map((res) => {
          const user = {
            uid: res.user.uid,
